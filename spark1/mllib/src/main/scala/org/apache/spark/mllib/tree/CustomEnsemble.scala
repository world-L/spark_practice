...
private class CustomEnsemble (
  ...
  require(numTrees > 0, s"CustomEnsemble requires numTrees > 0, but was given numTrees = $numTrees.")
  require(CustomEnsemble.supportedFeatureSubsetStrategies.contains(featureSubsetStrategy)
    || Try(featureSubsetStrategy.toInt).filter(_ > 0).isSuccess
    || Try(featureSubsetStrategy.toDouble).filter(_ > 0).filter(_ <= 1.0).isSuccess,
    s"CustomEnsemble given invalid featureSubsetStrategy: $featureSubsetStrategy." +
    s" Supported values: ${NewCEParams.supportedFeatureSubsetStrategies.mkString(", ")}," +
    s" (0.0-1.0], [1-n].")
   ...
   * @return CustomEnsembleModel that can be used for prediction.
   */
  def run(input: RDD[LabeledPoint]): CustomEnsembleModel = {
    val trees: Array[NewDTModel] = NewCustomEnsemble.run(input.map(_.asML), strategy, numTrees,
      featureSubsetStrategy, seed.toLong, None)
    new CustomEnsembleModel(strategy.algo, trees.map(_.toOld))
  }

}

@Since("1.2.0")
object CustomEnsemble extends Serializable with Logging {
  ...
  def trainClassifier(
      ...
      numTrees: Int,
      featureSubsetStrategy: String,
      seed: Int): CustomEnsembleModel = {
    require(strategy.algo == Classification,
      s"CustomEnsemble.trainClassifier given Strategy with invalid algo: ${strategy.algo}")
    val rf = new CustomEnsemble(strategy, numTrees, featureSubsetStrategy, seed)
    rf.run(input)
  }
  ...
  def trainClassifier(
      ...
      maxDepth: Int,
      maxBins: Int,
      seed: Int = Utils.random.nextInt()): CustomEnsembleModel = {
  ...
  def trainRegressor(
    ...
    require(strategy.algo == Regression,
      s"CustomEnsemble.trainRegressor given Strategy with invalid algo: ${strategy.algo}")
    val rf = new CustomEnsemble(strategy, numTrees, featureSubsetStrategy, seed)
    rf.run(input)
  }
  ...
  /**
   * Java-friendly API for [[org.apache.spark.mllib.tree.CustomEnsemble$#trainRegressor]]
   */
  @Since("1.2.0")
  def trainRegressor(
      input: JavaRDD[LabeledPoint],
   ...
   */
  @Since("1.2.0")
  val supportedFeatureSubsetStrategies: Array[String] = NewCEParams.supportedFeatureSubsetStrategies
}
