 ...
 */
private[spark] object CustomEnsemble extends Logging {
  ...
  def run(
    ...
    while (nodeStack.nonEmpty) {
      ...
      val (nodesForGroup, treeToNodeToIndexInfo) =
        CustomEnsemble.selectNodesToSplit(nodeStack, maxMemoryUsage, metadata, rng)
      // Sanity check (should never occur):
      assert(nodesForGroup.nonEmpty,
        s"CustomEnsemble selected empty nodesForGroup.  Error for unknown reason.")
      ...
      CustomEnsemble.findBestSplits(baggedInput, metadata, topNodesForGroup, nodesForGroup,
  ...
  private[tree] def selectNodesToSplit(
    ...
    while (nodeStack.nonEmpty && (memUsage < maxMemoryUsage || memUsage == 0)) {
      ...
      val nodeMemUsage = CustomEnsemble.aggregateSizeForNode(metadata, featureSubset) * 8L
